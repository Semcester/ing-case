...
class UpdateConfirmationModal extends LitElement {
  static styles = css`
    :host {
      display: none;
      position: fixed;
  ...
  render() {
    return html`
      <div class="modal-content">
        ...
        <p>
          ${translate("updateConfirmMessage").replace(
            "{employeeName}",
            this.employeeName,
          )}
        </p>
  ...
  _confirm() {
    this.dispatchEvent(
      new CustomEvent("confirm-update", { bubbles: true, composed: true }),
    );
...
customElements.define("update-confirmation-modal", UpdateConfirmationModal);
